...
namespace GatewayNet.Controllers
{
    public class ToursProtoController : ToursService.ToursServiceBase
    {
        ...
        public override async Task<DeleteCheckpointResponse> DeleteCheckpoint(DeleteCheckpointRequest request, ServerCallContext context)
        {
            try
            {
                _logger.LogInformation($"Forwarding DeleteCheckpoint request for checkpoint {request.CheckpointId} to tours service");
                ...
                var client = new ToursService.ToursServiceClient(channel);
                var response = await client.DeleteCheckpointAsync(request);

                _logger.LogInformation($"Delete checkpoint result: {response.Success}");

                return response;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error forwarding DeleteCheckpoint request");
        ...
        public override async Task<ArchiveTourResponse> ArchiveTour(ArchiveTourRequest request, ServerCallContext context)
        {
            try
            {
                _logger.LogInformation($"Forwarding ArchiveTour request for tour {request.TourId} to tours service");
                ...
                _logger.LogInformation($"Archive tour result: {response.Success}");

                return response;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error forwarding ArchiveTour request");
